...
class ArmaSerializer(serializers.HyperlinkedModelSerializer, NestedUpdateMixin):
    ...
    def update(self, instance, validated_data):
        calibre_data = validated_data.pop('calibre')
        new_calibre = Calibre.objects.filter(desc_calibre=calibre_data['desc_calibre']).first()
        instance.calibre = new_calibre
        instance.marca = validated_data.get('marca', instance.marca)
        instance.modelo = validated_data.get('modelo', instance.modelo)
        instance.quantidade_de_tiros = validated_data.get('quantidade_de_tiros', instance.quantidade_de_tiros)
        instance.valor_estimado = validated_data.get('valor_estimado', instance.valor_estimado)
